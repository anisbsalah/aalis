...
for locale in "${LOCALES[@]}"; do
	# sed -i "/[#[:space:]]*${locale}.UTF-8/s/^[#[:space:]]*//g" /etc/locale.gen
	sed -i "s/^[#[:space:]]*${locale}.UTF-8/${locale}.UTF-8/" /etc/locale.gen
done
# Generate the locale
locale-gen
...
echo "FONT=${CONSOLEFONT}" | tee -a /etc/vconsole.conf
...
if [[ "$(whoami)" == root ]]; then
	...
	# Copy 'aalis' project to user's home directory
	cp -R "${HOME}/aalis" "/home/${USERNAME}/"
	chown -R "${USERNAME}": "/home/${USERNAME}/aalis"
	echo "> 'aalis' project copied to the user's home directory."
